...
template <typename T> struct ScriptDeducer
{
	/*enum { FIELD_TYPE = FIELD_TYPEUNKNOWN };*/
};
...
#ifdef MAPBASE_VSCRIPT
	virtual bool Get( void* p, const char* pszKey, ScriptVariant_t& variant )
	{
		return false;
	}
	virtual bool Set( void* p, const char* pszKey, ScriptVariant_t& variant )
	{
		return false;
	}

	virtual ScriptVariant_t* Add( void* p, ScriptVariant_t& variant )
	{
		return NULL;
	}
	virtual ScriptVariant_t* Subtract( void* p, ScriptVariant_t& variant )
	{
		return NULL;
	}
	virtual ScriptVariant_t* Multiply( void* p, ScriptVariant_t& variant )
	{
		return NULL;
	}
	virtual ScriptVariant_t* Divide( void* p, ScriptVariant_t& variant )
	{
		return NULL;
	}
#endif
...
struct ScriptVariant_t
{
	...
	{
		Assert( m_type == FIELD_VECTOR );
		static QAngle vecNull( 0, 0, 0 );
		return ( m_pAngle ) ? *m_pAngle : vecNull;
	}
#endif
	...
	void Free()
	{
		...
		{
			delete m_pszString;    // Generally only needed for return results
		}
	}
...
template <> inline ScriptClassDesc_t* GetScriptDesc<ScriptNoBase_t>( ScriptNoBase_t* )
{
	return NULL;
}
...
public:
	...
	template <typename T> HSCRIPT RegisterInstance( T* pInstance, bool bAllowDestruct = false )
	{
		return RegisterInstance( GetScriptDesc( pInstance ), pInstance, bAllowDestruct );
	}
	template <typename T> HSCRIPT RegisterInstance( T* pInstance, const char* pszInstance, HSCRIPT hScope = NULL, bool bAllowDestruct = false )
	{
		HSCRIPT hInstance = RegisterInstance( GetScriptDesc( pInstance ), pInstance, bAllowDestruct );
...
struct ScriptHook_t
{
	...
	void AddParameter( const char* pszName, ScriptDataType_t type )
	{
		...
		m_desc.m_Parameters.EnsureCapacity( iCur + 1 );
		m_desc.m_Parameters.AddToTail( type );
		m_pszParameterNames.SetGrowSize( 1 );
		m_pszParameterNames.EnsureCapacity( iCur + 1 );
		m_pszParameterNames.AddToTail( pszName );
	}
