...
struct virtualmodel_t
{
	...
	{
		return &m_group[ m_anim[ animation ].group ];    // Note: user must manage mutex for this
	}
...
struct studiohdr2_t
{
	...
	{
		return flMaxEyeDeflection != 0.0f ? flMaxEyeDeflection : 0.866f;    // default to cos(30) if not set
	}
...
//public:
	...
	inline float		MaxEyeDeflection() const
	{
		return studiohdr2index ? pStudioHdr2()->MaxEyeDeflection() : 0.866f;    // default to cos(30) if not set
	}
