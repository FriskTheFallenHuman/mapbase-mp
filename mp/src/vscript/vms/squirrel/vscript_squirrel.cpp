...
public:
	...
	// Do not implicitly add/remove ref in conversion to SQObjectPtr
	// While harmless for write, it is unnecessary
...
int SquirrelVM::GetKeyValue( HSCRIPT hScope, int nIterator, ScriptVariant_t* pKey, ScriptVariant_t* pValue )
{
	...
	if( SQ_SUCCEEDED( sq_next( vm_, -2 ) ) )
	{
		if( pKey )
		{
			getVariant( vm_, -2, *pKey );
		}
		if( pValue )
		{
			getVariant( vm_, -1, *pValue );
		}
...
void SquirrelVM::WriteObject( const SQObjectPtr& obj, CUtlBuffer* pBuffer, WriteStateMap& writeState )
{
	...
	switch( obj._type )
	{
		...
		case OT_TABLE:
		{
			...
			DBG_CODE_NOSCOPE( void* pCountUndo = pBuffer->PeekPut(); )
			...
			if( count )
			{
				int prev = *( int* )pCountUndo;
				*( int* )pCountUndo = prev - count;
		...
		case OT_ARRAY:
		{
			...
			DBG_CODE_NOSCOPE(
				int counter = count;
				void* pCountUndo = pBuffer->PeekPut();
			)
			pBuffer->PutInt( count );
		...
		case OT_GENERATOR:
		{
			...
			Assert( ci._ip && ci._ip >= pThis->_closure._unVal.pClosure->_function->_instructions );
			int offset = ( int )ci._ip - ( int )pThis->_closure._unVal.pClosure->_function->_instructions;
			...
			count = pThis->_etraps.size();
			pBuffer->PutInt( count );
			for( int i = 0; i < count; ++i )
			{
				const SQExceptionTrap& et = pThis->_etraps[i];
...
void SquirrelVM::ReadObject( SQObjectPtr& pObj, CUtlBuffer* pBuffer, ReadStateMap& readState )
{
	...
	switch( obj._type )
	{
		...
		case OT_BOOL:
			obj._unVal.nInteger = ( bool )( pBuffer->GetChar() == 1 );
			break;
		...
		case OT_NATIVECLOSURE:
		{
			...
			{
				Warning( "'%s' is not nativeclosure\n", psz );
			}
#endif
		...
		case OT_FUNCPROTO:
		{
			...
			pThis->_bgenerator = ( pBuffer->GetChar() == 1 );
		...
		case OT_THREAD:
		{
			...
			SQVM* pThis = sq_newthread( vm_, MIN_STACK_OVERHEAD << 2 );
			vm_->Pop();
		...
		case OT_GENERATOR:
		{
			...
			SQVM::CallInfo& ci = pThis->_ci;
			{
				...
				int size = sizeof( SQInstruction ) * closure._unVal.pClosure->_function->_ninstructions;
			...
			count = pBuffer->GetInt();
			pThis->_etraps.resize( count );
			for( int i = 0; i < count; ++i )
			{
				SQExceptionTrap& et = pThis->_etraps[i];
...
void SquirrelVM::WriteVM( SQVM* pThis, CUtlBuffer* pBuffer, WriteStateMap& writeState )
{
	...
	if( int c = sq_collectgarbage( pThis ) > 0 )
	{
