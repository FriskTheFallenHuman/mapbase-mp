...
private:
	...
	int				GetNumMapBrushes()
	{
		return nummapbrushes;
	}

	const Vector&	GetEntityOrigin( int idx )
	{
		return ( idx < num_entities && idx >= 0 ) ? entities[idx].origin : vec3_origin;
	}
	int				GetEntityFirstBrush( int idx )
	{
		return ( idx < num_entities && idx >= 0 ) ? entities[idx].firstbrush : 0;
	}
	int				GetEntityNumBrushes( int idx )
	{
		return ( idx < num_entities && idx >= 0 ) ? entities[idx].numbrushes : 0;
	}
