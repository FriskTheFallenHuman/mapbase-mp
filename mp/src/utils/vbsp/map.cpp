...
int CMapFile::ScriptAddSimpleEntityKV( HSCRIPT hKV )
{
	...
	while( ( nIterator = g_pScriptVM->GetKeyValue( hKV, nIterator, &varKey, &varValue ) ) != -1 )
	{
		...
		{
			case FIELD_CSTRING:
				Q_strncpy( szValue, varValue.m_pszString, sizeof( szValue ) );
				break;
			case FIELD_INTEGER:
				Q_snprintf( szValue, sizeof( szValue ), "%i", varValue.m_int );
				break;
			case FIELD_FLOAT:
				Q_snprintf( szValue, sizeof( szValue ), "%f", varValue.m_float );
				break;
			case FIELD_CHARACTER:
				Q_snprintf( szValue, sizeof( szValue ), "%c", varValue.m_char );
				break;
			case FIELD_BOOLEAN:
				Q_snprintf( szValue, sizeof( szValue ), "%d", varValue.m_bool );
				break;
			case FIELD_VECTOR:
				Q_snprintf( szValue, sizeof( szValue ), "%f %f %f", ( *varValue.m_pVector ).x, ( *varValue.m_pVector ).y, ( *varValue.m_pVector ).z );
				break;
			default:
				szValue[0] = '\0';
				break;
