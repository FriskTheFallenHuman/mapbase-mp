...
void ParseScriptTableKeyValues( CBaseEntity* pEntity, HSCRIPT hKV )
{
	...
	while( ( nIterator = g_pScriptVM->GetKeyValue( hKV, nIterator, &varKey, &varValue ) ) != -1 )
	{
		...
		{
			case FIELD_CSTRING:
				pEntity->KeyValue( varKey.m_pszString, varValue.m_pszString );
				break;
			case FIELD_INTEGER:
				pEntity->KeyValueFromInt( varKey.m_pszString, varValue.m_int );
				break;
			case FIELD_FLOAT:
				pEntity->KeyValue( varKey.m_pszString, varValue.m_float );
				break;
			case FIELD_VECTOR:
				pEntity->KeyValue( varKey.m_pszString, *varValue.m_pVector );
				break;
			case FIELD_HSCRIPT:
			{
...
bool ScriptMatcherMatch( const char* pszQuery, const char* szValue )
{
