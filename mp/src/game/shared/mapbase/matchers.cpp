...
static bool ResponseSystemCompare( const char* criterion, const char* value )
{
	return Matcher_NamesMatch( criterion, value );
}
#else
...
{
	return Matcher_Match( pszQuery, CNumStr( iValue ) );
}
bool Matcher_Match( const char* pszQuery, float flValue )
{
	return Matcher_Match( pszQuery, CNumStr( flValue ) );
}
