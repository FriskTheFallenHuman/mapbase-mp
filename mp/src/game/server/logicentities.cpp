...
public:
	...
	inline void LCMsg( const char* msg, const char* append = NULL )
	{
		ConColorMsg( m_MsgColor, msg, append );
	}
	inline void LCDevMsg( int lvl, const char* msg, const char* append = NULL )
	{
		developer.GetInt() >= lvl ? ConColorMsg( m_MsgColor, msg, append ) : ( void )0;
	}
	inline void LCWarning( const char* msg, const char* append = NULL )
	{
		ConColorMsg( m_WarningColor, msg, append );
	}
	inline void LCDevWarning( int lvl, const char* msg, const char* append = NULL )
	{
		developer.GetInt() >= lvl ? ConColorMsg( m_WarningColor, msg, append ) : ( void )0;
	}
	...
	// Inputs
	void InputSendMsg( inputdata_t& inputdata )
	{
		!m_bNewLineNotAuto ? LCMsg( "%s\n", inputdata.value.String() ) : LCMsg( "%s", inputdata.value.String() );
	}
	void InputSendWarning( inputdata_t& inputdata )
	{
		!m_bNewLineNotAuto ? LCWarning( "%s\n", inputdata.value.String() ) : LCWarning( "%s", inputdata.value.String() );
	}
	void InputSendDevMsg( inputdata_t& inputdata )
	{
		!m_bNewLineNotAuto ? LCDevMsg( m_iDevLevel, "%s\n", inputdata.value.String() ) : LCDevMsg( m_iDevLevel, "%s", inputdata.value.String() );
	}
	void InputSendDevWarning( inputdata_t& inputdata )
	{
		!m_bNewLineNotAuto ? LCDevWarning( m_iDevLevel, "%s\n", inputdata.value.String() ) : LCDevWarning( m_iDevLevel, "%s", inputdata.value.String() );
	}

	void InputNewLine( inputdata_t& inputdata )
	{
		LCMsg( "\n" );
	}
	void InputDevNewLine( inputdata_t& inputdata )
	{
		LCDevMsg( m_iDevLevel, "\n" );
	}
	...
	{
		UTIL_GetLocalPlayer() ? engine->ClientCommand( UTIL_GetLocalPlayer()->edict(), "clear" ) : ( void )0;
	}
...
private:
	...
	void InputSetX( inputdata_t& inputdata )
	{
		SetCoordinate( inputdata.value.Float(), 'X', inputdata.pActivator );
	}
	void InputSetY( inputdata_t& inputdata )
	{
		SetCoordinate( inputdata.value.Float(), 'Y', inputdata.pActivator );
	}
	void InputSetZ( inputdata_t& inputdata )
	{
		SetCoordinate( inputdata.value.Float(), 'Z', inputdata.pActivator );
	}
	void InputGetX( inputdata_t& inputdata )
	{
		GetCoordinate( 'X', inputdata.pActivator );
	}
	void InputGetY( inputdata_t& inputdata )
	{
		GetCoordinate( 'Y', inputdata.pActivator );
	}
	void InputGetZ( inputdata_t& inputdata )
	{
		GetCoordinate( 'Z', inputdata.pActivator );
	}
	void InputAddX( inputdata_t& inputdata )
	{
		AddCoordinate( inputdata.value.Float(), 'X', inputdata.pActivator );
	}
	void InputAddY( inputdata_t& inputdata )
	{
		AddCoordinate( inputdata.value.Float(), 'Y', inputdata.pActivator );
	}
	void InputAddZ( inputdata_t& inputdata )
	{
		AddCoordinate( inputdata.value.Float(), 'Z', inputdata.pActivator );
	}
	void InputSubtractX( inputdata_t& inputdata )
	{
		SubtractCoordinate( inputdata.value.Float(), 'X', inputdata.pActivator );
	}
	void InputSubtractY( inputdata_t& inputdata )
	{
		SubtractCoordinate( inputdata.value.Float(), 'Y', inputdata.pActivator );
	}
	void InputSubtractZ( inputdata_t& inputdata )
	{
		SubtractCoordinate( inputdata.value.Float(), 'Z', inputdata.pActivator );
	}
...
void CMathVector::GetCoordinate( char coord, CBaseEntity* pActivator )
{
	...
	{
		case 'X':
			m_OnGetX.Set( vec.x, pActivator, this );
			break;
		case 'Y':
			m_OnGetY.Set( vec.y, pActivator, this );
			break;
		case 'Z':
			m_OnGetZ.Set( vec.z, pActivator, this );
			break;
	}
}
...
private:
	...
	// Inputs
	void InputGetCurtime( inputdata_t& inputdata )
	{
		m_OutCurtime.Set( gpGlobals->curtime, inputdata.pActivator, this );
	}
	void InputGetFrameCount( inputdata_t& inputdata )
	{
		m_OutFrameCount.Set( gpGlobals->framecount, inputdata.pActivator, this );
	}
	void InputGetFrametime( inputdata_t& inputdata )
	{
		m_OutFrametime.Set( gpGlobals->frametime, inputdata.pActivator, this );
	}
	void InputGetTickCount( inputdata_t& inputdata )
	{
		m_OutTickCount.Set( gpGlobals->tickcount, inputdata.pActivator, this );
	}
	void InputGetIntervalPerTick( inputdata_t& inputdata )
	{
		m_OutIntervalPerTick.Set( TICK_INTERVAL, inputdata.pActivator, this );
	}
...
Vector CLogicEntityPosition::GetPosition( CBaseEntity* pEntity )
{
	switch( m_iPositionType )
	{
		...
		case POSITION_BBOX:
			return pEntity->WorldSpaceCenter();
		case POSITION_EYES:
			return pEntity->EyePosition();
		case POSITION_EARS:
			return pEntity->EarPosition();
...
QAngle CLogicEntityPosition::GetAngles( CBaseEntity* pEntity )
{
	switch( m_iPositionType )
	{
		...
		case POSITION_ORIGIN:
			...
			break;
		case POSITION_LOCAL:
			return pEntity->GetLocalAngles();
			break;
		case POSITION_EYES:
			return pEntity->EyeAngles();
			break;
		case POSITION_ATTACHMENT:
		{
			CBaseAnimating* pAnimating = pEntity->GetBaseAnimating();
...
private:
	...
	inline bool VerifyPatternValid()
	{
		return ( m_iszPattern != NULL_STRING && STRING( m_iszPattern )[0] != '\0' );
	}
