	...
	*/
	...
	switch( FieldType() )
	{
		...
		case FIELD_COLOR32:
			...
			break;
		default:
			fieldtype = UTIL_VarArgs( "unknown: %i", FieldType() );
	}
...
void variant_t::SetScriptVariant( ScriptVariant_t& var )
{
	switch( FieldType() )
	{
		...
		case FIELD_POSITION_VECTOR:
		case FIELD_VECTOR:
			var = reinterpret_cast<Vector*>( &flVal );
			break; // HACKHACK
		case FIELD_BOOLEAN:
			var = bVal;
			break;
...
bool Variant_Equal( variant_t val1, variant_t val2, bool bLenAllowed )
{
	...
	switch( val1.FieldType() )
	{
		...
		case FIELD_EHANDLE:
			return val1.Entity() == val2.Entity();
		case FIELD_VECTOR:
		{
			Vector vec1;
			val1.Vector3D( vec1 );
		...
		default:
			return Matcher_NamesMatch_MutualWildcard( val1.String(), val2.String() );
#else
		default:
			return FStrEq( val1.String(), val2.String() );
#endif
	}
...
bool Variant_Greater( variant_t val1, variant_t val2, bool bLenAllowed )
{
	...
	switch( val1.FieldType() )
	{
		...
		default:
			return strlen( val1.String() ) > strlen( val2.String() );
	}
