...
public:
	...
	{
		return false;    // Always ragdoll, unless specified by the leaf class
	}
	...
	{
		return m_iDamageCount;    // # of times NPC has been damaged.  used for tracking 1-shot kills.
	}
...
public:
...
#ifdef MAPBASE
	inline bool			OverridingWeaponProficiency()
	{
		return ( m_ProficiencyOverride > WEAPON_PROFICIENCY_INVALID );
	}
#endif
	...
	int					ScriptGetCurrentWeaponProficiency()
	{
		return GetCurrentWeaponProficiency();
	}
	...
	bool				ScriptInViewCone( const Vector& vecSpot )
	{
		return FInViewCone( vecSpot );
	}
	bool				ScriptEntInViewCone( HSCRIPT pEntity )
	{
		return FInViewCone( ToEnt( pEntity ) );
	}

	bool				ScriptInAimCone( const Vector& vecSpot )
	{
		return FInAimCone( vecSpot );
	}
	bool				ScriptEntInAimCone( HSCRIPT pEntity )
	{
		return FInAimCone( ToEnt( pEntity ) );
	}

	const Vector&		ScriptBodyAngles( void )
	{
		static Vector vec;
		QAngle qa = BodyAngles();
		vec.x = qa.x;
		vec.y = qa.y;
		vec.z = qa.z;
		return vec;
	...
	{
		return m_lastNavArea;    // return the last nav area the player occupied - NULL if unknown
	}
