...
public:
	...
	Vector		ScriptGetNodePosition( int nodeID )
	{
		return GetNodePosition( HULL_HUMAN, nodeID );
	}
	Vector		ScriptGetNodePositionWithHull( int nodeID, int hull )
	{
		return GetNodePosition( ( Hull_t )hull, nodeID );
	}

	int			ScriptNearestNodeToPoint( const Vector& vecPosition, bool bCheckVisibility = true )
	{
		return NearestNodeToPoint( NULL, vecPosition, bCheckVisibility );
	}
