...
public:
	...
	{
		m_bForceConditionsGather = true;    // Force an NPC out of PVS to call GatherConditions on next think
		SetEfficiency( AIE_NORMAL );
	}
	bool				IsForceGatherConditionsSet()
	{
...
#ifdef MAPBASE
	Activity			GetTranslatedActivity( void )
	{
		return m_translatedActivity;
	}
	Activity			GetIdealTranslatedActivity( void )
	{
		return m_IdealTranslatedActivity;
	}
#endif
...
public:
	...
	{
		return 0;    // Enemy distances within this tolerance of each other are considered equivalent.
	}
...
public:
	...
	virtual Vector		GetAltFireTarget()
	{
		return GetEnemy() ? GetEnemy()->BodyTarget( Weapon_ShootPosition() ) : vec3_origin;
	}
...
private:
	...
	void				VScriptWake( HSCRIPT hActivator )
	{
		Wake( ToEnt( hActivator ) );
	}
	void				VScriptSleep()
	{
		Sleep();
	...
	int					VScriptGetSleepState()
	{
		return ( int )GetSleepState();
	}
	void				VScriptSetSleepState( int sleepState )
	{
		SetSleepState( ( AI_SleepState_t )sleepState );
	}

	const char*			VScriptGetHintGroup()
	{
		return STRING( GetHintGroup() );
	}
	HSCRIPT				VScriptGetHintNode();

	const char*			ScriptGetActivity()
	{
		return GetActivityName( GetActivity() );
	}
	int					ScriptGetActivityID()
	{
		return GetActivity();
	}
	void				ScriptSetActivity( const char* szActivity )
	{
		SetActivity( ( Activity )GetActivityID( szActivity ) );
	}
	void				ScriptSetActivityID( int iActivity )
	{
		SetActivity( ( Activity )iActivity );
	}
	int					ScriptTranslateActivity( const char* szActivity )
	{
		return TranslateActivity( ( Activity )GetActivityID( szActivity ) );
	}
	int					ScriptTranslateActivityID( int iActivity )
	{
		return TranslateActivity( ( Activity )iActivity );
	}

	const char*			VScriptGetGestureVersionOfActivity( const char* pszActivity )
	{
		return GetActivityName( GetGestureVersionOfActivity( ( Activity )GetActivityID( pszActivity ) ) );
	}
	int					VScriptGetGestureVersionOfActivityID( int iActivity )
	{
		return GetGestureVersionOfActivity( ( Activity )iActivity );
	}
	const char*			VScriptGetSequenceVersionOfGesture( const char* pszActivity )
	{
		return GetActivityName( GetSequenceVersionOfGesture( ( Activity )GetActivityID( pszActivity ) ) );
	}
	int					VScriptGetSequenceVersionOfGestureID( int iActivity )
	{
		return GetSequenceVersionOfGesture( ( Activity )iActivity );
	}

	const char*			VScriptGetSchedule();
	int					VScriptGetScheduleID();
	void				VScriptSetSchedule( const char* szSchedule );
	void				VScriptSetScheduleID( int iSched )
	{
		SetSchedule( iSched );
	}
	...
	bool				VScriptHasConditionID( int iCondition )
	{
...
public:
	...
	inline void SetShotRegulator( CAI_ShotRegulator NewRegulator )
	{
		m_ShotRegulator = NewRegulator;
	}
#endif
...
public:
	...
	{
		SetHintGroup( inputdata.value.StringID() );
	}
#endif
	...
	{
		return 1024;    // Default cover radius
	}
