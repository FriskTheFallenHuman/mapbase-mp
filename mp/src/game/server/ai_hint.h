...
#ifdef MAPBASE // From Alien Swarm SDK
	void		SetFilterFunc( HintSearchFilterFunc_t pfnFilter, void* pContext = NULL )
	{
		m_pfnFilter = pfnFilter;
		m_pFilterContext = pContext;
	}
	bool		PassesFilter( CAI_Hint* pCandidate ) const
	{
		return ( m_pfnFilter ) ? ( *m_pfnFilter )( m_pFilterContext, pCandidate ) : true;
	}
#endif
...
public:
	...
	int					GetTargetWCNodeID( void ) const
	{
		return m_NodeData.nTargetWCNodeID;
	}
	int					GetWCNodeID( void ) const
	{
	...
	int					GetRadius() const
	{
		return m_NodeData.nRadius;    // From Alien Swarm SDK
	}

	float				GetHintWeight() const
	{
		return m_NodeData.flWeight;
	}
	float				GetHintWeightInverse() const
	{
		return m_NodeData.flWeightInverse;    // Used to multiply distances
	}
#endif
	...
	int					ScriptGetHintType()
	{
		return ( int )HintType();
	}
	HSCRIPT				ScriptGetUser()
	{
		return ToHScript( User() );
	}
	const char*			ScriptGetHintGroup()
	{
		return STRING( GetGroup() );
	}
	const char*			ScriptGetHintActivity()
	{
		return STRING( HintActivityName() );
	}
#endif
