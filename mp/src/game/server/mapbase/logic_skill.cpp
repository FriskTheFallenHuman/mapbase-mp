...
private:
	...
	void InputStartListening( inputdata_t& inputdata )
	{
		m_bListeningForSkillChanges = true;
	}
	void InputStopListening( inputdata_t& inputdata )
	{
		m_bListeningForSkillChanges = false;
	}

	// Used by gamerules to fire OnSkillChanged.
	// Passes the level it changed to as well.
	void InputSkillLevelChanged( inputdata_t& inputdata )
	{
		m_bListeningForSkillChanges ? m_OnSkillChanged.Set( inputdata.value.Int(), inputdata.pActivator, this ) : ( void )0;
	}
				 ...
				 void CLogicSkill::InputTest( inputdata_t& inputdata )
{
	...
	{
		case SKILL_EASY:
			m_OnEasy.FireOutput( this, this );
			break;
		case SKILL_MEDIUM:
			m_OnMedium.FireOutput( this, this );
			break;
		case SKILL_HARD:
			m_OnHard.FireOutput( this, this );
			break;
	}
}
