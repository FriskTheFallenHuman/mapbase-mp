...
public:
	...
	void InputGetConditionName( inputdata_t& inputdata )
	{
		m_OutConditionName.Set( AllocPooledString( ConditionName( inputdata.value.Int() ) ), inputdata.pActivator, this );
	}
	void InputGetScheduleName( inputdata_t& inputdata )
	{
		m_OutScheduleName.Set( AllocPooledString( ScheduleName( inputdata.value.Int() ) ), inputdata.pActivator, this );
	}
	...
	{
		SetCondition( TranslateConditionString( inputdata.value.String() ) );
	}
	void InputClearCondition( inputdata_t& inputdata )
	{
		ClearCondition( TranslateConditionString( inputdata.value.String() ) );
	}
#if AI_MONITOR_USE_UTLVECTOR
	void InputClearAllConditions( inputdata_t& inputdata )
	{
	...
	void InputSetSchedule( inputdata_t& inputdata )
	{
		SetSchedule( TranslateScheduleString( inputdata.value.String() ) );
	}
	void InputClearSchedule( inputdata_t& inputdata )
	{
		ClearSchedule( TranslateScheduleString( inputdata.value.String() ) );
	}
#if AI_MONITOR_USE_UTLVECTOR
	void InputClearAllSchedules( inputdata_t& inputdata )
	{
	...
	void InputClearAllSchedules( inputdata_t& inputdata )
	{
		m_Schedules.ClearAll();
	}
#endif

	void InputSetHint( inputdata_t& inputdata )
	{
		SetHint( inputdata.value.Int() );
	}
	void InputClearHint( inputdata_t& inputdata )
	{
		ClearHint( inputdata.value.Int() );
	}
	void InputClearAllHints( inputdata_t& inputdata )
	{
		m_Hints.RemoveAll();
	}
...
#define GetCooldownTime() (m_flCooldownTime != -1 ? m_flCooldownTime : GetThinkTime())
...
#if AI_MONITOR_USE_UTLVECTOR
	inline void			SetCondition( int iCondition )
	{
		m_Conditions.HasElement( iCondition ) ? NULL : m_Conditions.AddToTail( iCondition );
	}
	inline void			ClearCondition( int iCondition )
	{
		m_Conditions.FindAndRemove( iCondition );
	}
	inline bool			HasCondition( int iCondition )
	{
		return m_Conditions.HasElement( iCondition );
	}
#else
	inline void			SetCondition( int iCondition )
	{
		m_Conditions.Set( iCondition );
	}
	inline void			ClearCondition( int iCondition )
	{
		m_Conditions.Clear( iCondition );
	}
	inline bool			HasCondition( int iCondition )
	{
		return m_Conditions.IsBitSet( iCondition );
	}
#endif

	static int			GetConditionID( const char* condName )
	{
		return CAI_BaseNPC::GetSchedulingSymbols()->ConditionSymbolToId( condName );
	}
	const char*			ConditionName( int conditionID );

	int					TranslateConditionString( const char* condName );
	inline int			ConditionLocalToGlobal( CAI_BaseNPC* pTarget, int conditionID )
	{
		return pTarget->GetClassScheduleIdSpace()->ConditionLocalToGlobal( conditionID );
	}
...
#if AI_MONITOR_USE_UTLVECTOR
	inline void			SetSchedule( int iSchedule )
	{
		m_Schedules.HasElement( iSchedule ) ? NULL : m_Schedules.AddToTail( iSchedule );
	}
	inline void			ClearSchedule( int iSchedule )
	{
		m_Schedules.FindAndRemove( iSchedule );
	}
	inline bool			HasSchedule( int iSchedule )
	{
		return m_Schedules.HasElement( iSchedule );
	}
#else
	inline void			SetSchedule( int iSchedule )
	{
		m_Schedules.Set( iSchedule );
	}
	inline void			ClearSchedule( int iSchedule )
	{
		m_Schedules.Clear( iSchedule );
	}
	inline bool			HasSchedule( int iSchedule )
	{
		return m_Schedules.IsBitSet( iSchedule );
	}
#endif

	static int			GetScheduleID( const char* schedName )
	{
		return CAI_BaseNPC::GetSchedulingSymbols()->ScheduleSymbolToId( schedName );
	}
	const char*			ScheduleName( int scheduleID );

	int					TranslateScheduleString( const char* schedName );
	inline int			ScheduleLocalToGlobal( CAI_BaseNPC* pTarget, int scheduleID )
	{
		return pTarget->GetClassScheduleIdSpace()->ScheduleLocalToGlobal( scheduleID );
	}
	...
	inline void			SetHint( int iHint )
	{
		m_Hints.HasElement( iHint ) ? NULL : m_Hints.AddToTail( iHint );
	}
	inline void			ClearHint( int iHint )
	{
		m_Hints.FindAndRemove( iHint );
	}
	inline bool			HasHint( int iHint )
	{
		return m_Hints.HasElement( iHint );
	}
