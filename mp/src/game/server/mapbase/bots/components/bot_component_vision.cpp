...
void CBotVision::Process()
{
	...
	else
	{
		...
		//limit angles
		if( accel > maxLookAccel )
		{
			accel = maxLookAccel;
		}
		else if( accel < -maxLookAccel )
		{
			accel = -maxLookAccel;
		}
	...
	if( accelStiff > maxLookAccel )
	{
		accelStiff = maxLookAccel;
	}
	else if( accelStiff < -maxLookAccel )
	{
		accelStiff = -maxLookAccel;
	}
	...
	//limit angles
	float limit = 89.0f;
	if( eyeAngles.x < -limit )
	{
		eyeAngles.x = -limit;
	}
	else if( eyeAngles.x > limit )
	{
		eyeAngles.x = limit;
	}
