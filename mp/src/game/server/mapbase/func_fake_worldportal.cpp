...
public:
	...
	// Input handlers
	void InputSetTargetPlane( inputdata_t& inputdata )
	{
		m_hTargetPlane = inputdata.value.Entity();
		if( m_hTargetPlane )
		{
			m_target = m_hTargetPlane->GetEntityName();
		}
	}
	void InputSetTargetPlaneAngle( inputdata_t& inputdata )
	{
		Vector vec;
		inputdata.value.Vector3D( vec );
		m_PlaneAngles.Init( vec.x, vec.y, vec.z );
	}
	...
	{
		m_hFogController = inputdata.value.Entity();
		if( m_hFogController )
		{
			m_iszFogController = m_hFogController->GetEntityName();
		}
	}
