...
Activity CAI_BaseNPC::NPC_BackupActivity( Activity eNewActivity )
{
	...
	switch( eNewActivity )
	{
		case ACT_COVER_MED:
			...
			break;
#if EXPANDED_HL2_COVER_ACTIVITIES
		case ACT_RANGE_AIM_MED:
			eNewActivity = ACT_RANGE_AIM_LOW;
			break;
		case ACT_RANGE_ATTACK1_MED:
...
Activity CAI_BaseNPC::NPC_TranslateActivity( Activity eNewActivity )
{
	...
	if( IsCrouching() )
	{
		switch( eNewActivity )
		{
			...
			case ACT_IDLE:
				eNewActivity = ACT_RANGE_AIM_LOW;
				break; // ACT_CROUCHIDLE is more-or-less deprecated and not friendly to weapon translation
		}
	}
	...
	if( modelKeyValues->LoadFromBuffer( modelinfo->GetModelName( GetModel() ), buf ) )
	{
		...
		if( pkvInteractions )
		{
			...
			while( pkvNode )
			{
				...
				if( pszTrigger )
					@@ -13111, 6 + 14744, 7 @@ void CAI_BaseNPC::ParseScriptedNPCInteractions( void )
					sInteraction.iFlags |= SCNPC_FLAG_NEEDS_WEAPON_THEM;
