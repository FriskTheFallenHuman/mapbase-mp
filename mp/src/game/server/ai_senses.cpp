...
{
	if( pSound->m_hOwner.Get() == GetOuterClass )
	{
		return false;
	}
	...
	float flHearDistanceSq = pSound->Volume() * GetOuterClass->HearingSensitivity();
	flHearDistanceSq *= flHearDistanceSq;
	if( pSound->GetSoundOrigin().DistToSqr( GetOuterClass->EarPosition() ) <= flHearDistanceSq )
	{
		return GetOuterClass->QueryHearSound( pSound );
	}
...
void CAI_Senses::Listen( void )
{
	...
	int iSoundMask = GetOuterClass->GetSoundInterests();

	if( iSoundMask != SOUND_NONE && !( GetOuterClass->HasSpawnFlags( SF_NPC_WAIT_TILL_SEEN ) ) )
	{
...
{
	if( pSightEnt == GetOuterClass || !pSightEnt->IsAlive() )
	{
		return false;
	}
	...
	if( !GetOuterClass->QuerySeeEntity( pSightEnt, true ) )
	{
		return false;
	}
...
{
#ifdef ENABLE_BOTS
	return GetOuterClass->IsAbleToSee( pSightEnt, CBaseCombatCharacter::USE_FOV );
#else
	return ( GetOuterClass->FInViewCone( pSightEnt ) && GetOuter()->FVisible( pSightEnt ) );
#endif
}
...
{
	return GetOuterClass->FVisibleThroughPortal( pPortal, pSightEnt );
}
#endif
...
{
	pSightEnt->m_pLink = GetOuterClass->m_pLink;
	GetOuterClass->m_pLink = pSightEnt;
}
...
{
	if( GetOuterClass->m_spawnflags & SF_NPC_WAIT_TILL_SEEN )
	{
		if( pSightEnt->IsPlayer() )
		{
			...
			if( pPlayer
					&& pPlayer->FInViewCone( GetOuterClass )
					&& FBoxVisible( pSightEnt, static_cast<CBaseEntity*>( GetOuterClass ), zero ) )
			{
				// player sees us, become normal now.
				GetOuterClass->m_spawnflags &= ~SF_NPC_WAIT_TILL_SEEN;
				return false;
			}
		}
		return true;
...
{
	GetOuterClass->OnSeeEntity( pSightEnt );
...
void CAI_Senses::EndGather( int nSeen, CUtlVector<EHANDLE>* pResult )
{
	...
	if( nSeen )
	{
		CBaseEntity* pCurrent = GetOuterClass->m_pLink;
...
void CAI_Senses::Look( int iDistance )
{
	...
	GetOuterClass->OnLooked( iDistance );
}
...
int CAI_Senses::LookForNPCs( int iDistance )
{
	...
	if( gpGlobals->curtime - m_TimeLastLookNPCs > timeNPCs )
	{
		...
		if( efficiency < AIE_SUPER_EFFICIENT )
		{
			...
			{
				if( ppAIs[i] != GetOuterClass && ( ppAIs[i]->ShouldNotDistanceCull() || origin.DistToSqr( ppAIs[i]->GetAbsOrigin() ) < distSq ) )
				{
					if( Look( ppAIs[i] ) )
					{
						nSeen++;
...
CSound* CAI_Senses::GetClosestSound( bool fScent, int validTypes, bool bUsePriority )
{
	...
	Vector earPosition = GetOuterClass->EarPosition();
	...
	while( pCurrent )
	{
		...
		{
			if( pCurrent->IsSoundType( validTypes ) && !GetOuterClass->ShouldIgnoreSound( pCurrent ) )
			{
				if( !bUsePriority || GetOuterClass->GetSoundPriority( pCurrent ) >= iBestPriority )
				{
					...
					if( flDist < flBestDist )
					{
						...
						iBestPriority = GetOuterClass->GetSoundPriority( pCurrent );
					}
				}
			}
		}
