...
#define ANIM_LAYER_NOEVENTS		0x0040
	...
	bool	NoEvents( void )
	{
		return ( ( m_fFlags & ANIM_LAYER_NOEVENTS ) != 0 );
	}
#endif
...
private:
	...
	int	ScriptAddGestureSequence( const char* pszSequence, bool autokill )
	{
		return AddGestureSequence( LookupSequence( pszSequence ), autokill );
	}
	int	ScriptAddGestureSequenceID( int iSequence, bool autokill )
	{
		return AddGestureSequence( iSequence, autokill );
	}
	...
	void ScriptSetLayerCycle( int iLayer, float flCycle )
	{
		SetLayerCycle( iLayer, flCycle );
	}
#endif
